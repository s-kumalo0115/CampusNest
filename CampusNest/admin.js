...
(function () {
  ...
  function initAdmin(adminUser) {
    ...
    // LANDLORDS
    const landlordsQuery = query(collection(db, "landlords"), orderBy("name"));
    onSnapshot(landlordsQuery, snapshot => {
      ...
      snapshot.forEach(docSnap => {
        ...
        tr.innerHTML = `
          ...
          <td>${verifiedText}</td>
          <td>
            <button class="btn-action btn-verify" data-id="${docSnap.id}" data-verified="${d.verified}">${toggleLabel}</button>
            <button class="btn-action btn-delete-landlord" data-id="${docSnap.id}">Delete</button>
          </td>
        `;
    ...
    onSnapshot(usersQuery, snapshot => {
      ...
      snapshot.forEach(docSnap => {
        ...
        tr.innerHTML = `
          <td>${escapeHtml(d.name || "-")}</td>
          <td>${escapeHtml(d.email || "-")}</td>
          <td><button class="btn-action btn-delete-user" data-id="${docSnap.id}">Delete</button></td>
        `;
    ...
    document.addEventListener("click", async (e) => {
      ...
      if (btnView) {
        ...
        if (!docSnap.exists()) { alert("Enquiry not found."); return; }
        const d = docSnap.data();
